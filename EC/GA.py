...
class GA(object):
    ...
    def __init__(self, opt, name, tar):
        ...
        self.f = open("results/face_data_"+name+".csv", 'a', newline="")
        os.system("mkdir " + opt.results + "/" + name)
    ...
    def sort(self):
        for i in range(len(self.pop)):
            if i >= self.opt.generation_count:
                self.pop = self.pop[:self.opt.generation_count]
                break
    ...
    def eval(self, individual, i):
        ...
        name = self.name + "/" + str(self.generation) + "_" + str(i)
        self.solver.test_save_imgs(img, name)
