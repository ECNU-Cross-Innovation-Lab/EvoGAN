...
class Solver(object):
    ...
    def test_save_imgs(self, face, src_name):

        face = Image.fromarray(face)
        saved_path = os.path.join(self.opt.results, "%s.jpg" % src_name)
        face.save(saved_path)

    def crop(self, img_path, tar_path):
    ...
    def prepare_img(self, img_path):
        ...
        img = self.test_dataset.dataset.img2tensor(img)
        img = torch.unsqueeze(img, 0)
        return img

    def run_wild(self, img_path, expression):
        img = self.prepare_img(img_path)
        with torch.no_grad():
            expression = torch.unsqueeze(torch.from_numpy(expression), 0)
            test_batch = {'src_img': img, 'tar_aus': expression}
            self.test_model.feed_batch(test_batch)
            self.test_model.forward()
